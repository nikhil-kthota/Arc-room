...
DROP POLICY IF EXISTS "Room creators can create folders" ON folders;
DROP POLICY IF EXISTS "Room creators can update their folders" ON folders;
DROP POLICY IF EXISTS "Room creators can delete their folders" ON folders;
...
-- Add constraint to prevent circular folder references (handle existing constraint)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints
    WHERE table_name = 'folders' AND constraint_name = 'check_no_self_reference'
  ) THEN
